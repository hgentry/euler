    ...
    for i in 0..sets.len() {
        ...
        for j in 0..i + 1 {
            ...
            if sets[j].post.len() > 0 {
                ...
                let mut new_changeset = ChangeSet {
                    pre: new_pre_set,
                    post: new_post_set,
                    modifier: OrdVar::new(0.0),
                };
                new_changeset.modifier =
                    OrdVar::new(calculate_modifier(&new_changeset.pre, &new_changeset.post));
                for k in i..sets.len() {
...
pub fn primes_as_changesets(max: i64) -> Vec<ChangeSet> {
    let mut primes = vec![];
    primes.push(ChangeSet {
        pre: vec![2],
        post: vec![],
        modifier: OrdVar::new(1.0f64),
    });
    primes.push(ChangeSet {
        pre: vec![3],
        post: vec![],
        modifier: OrdVar::new(1.5f64),
    });
    ...
    while checking <= max {
        ...
        if !failed {
            primes.push(ChangeSet {
                pre: vec![checking],
                post: vec![],
                modifier: OrdVar::new(checking as f64 / 2.0),
            });
        }
        failed = false;
        checking += 2;
...
pub fn generate_post_changesets(max: i64, primes: &Vec<ChangeSet>) -> Vec<ChangeSet> {
    ...
    for i in 2..max + 1 {
        ...
        posts.push(ChangeSet {
            pre: vec![],
            post: post_list,
            modifier: OrdVar::new(modifier),
        });
    }
    posts
}
