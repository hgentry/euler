...
pub fn solve() -> i64 {
	...
	//input = vec!(vec!(5,2,3), vec!(5,1,1), vec!(1,1,66));
	//input = vec!(vec!(1,2,3), vec!(4,5,6), vec!(3,2,1));
	let len = input.len();
	for i in 0..len {
		// Make sure the links are set up right for this i
		let mut g = build_graph(&input);
		let pos = i;
		for j in 0..pos {
			let n1 = g.get_node((j*len) as i64).clone();
			let n2 = g.get_node(((j+1)*len) as i64).clone();
			g.remove_directional_link(&n1,&n2);
		}
		for j in pos..len-1 {
			let n1 = g.get_node((j*len) as i64).clone();
			let n2 = g.get_node(((j+1)*len) as i64).clone();
			g.remove_directional_link(&n2,&n1);
		}
				...
				if n.outgoing.len() != 0 {
					...
					for n_j in 0..n.outgoing.len() {
						...
						if t2 == t2i {
							...
							//println!("{} -> {} = {}", n_i, n.outgoing[n_j], t+t2i);
							done = false;
						} else {
							g.update_node_content(&n2, min(t2, t + t2i));
							//println!("{} -> {} = {}", n_i, n.outgoing[n_j], min(t2, t + t2i));
							if t2 > t + t2i  {
								done = false;
							}
						}
						// g.remove_directional_link(&n, &n2);
					}
